...
// Owner numbers
const ownerNumber = ["923151914309"];
...
function loadLocalFiles() {
  ...
  if (fs.existsSync('./lib/azlan.html')) {
    console.log("✅ azlan.html found in lib folder");
  } else {
    console.log("⚠️ azlan.html not found in lib folder");
  }
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("KIRA-MD~")) {
    console.log("❌ Invalid CUTE-MARKHOR-MD session format");
    process.exit(1);
  }
  ...
  console.log("♻️ CUTE-MARKHOR-MD session restored successfully");
}
...
// Serve HTML page
app.get('/', (req, res) => {
  const htmlPath = path.join(__dirname, 'lib', 'azlan.html');
  if (fs.existsSync(htmlPath)) {
    ...
    res.send(`
      ...
      <html>
      <head>
        <title>CUTE-MARKHOR-MD Bot</title>
        <style>
...
app.get('/lib/azlan.html', (req, res) => {
  res.redirect('/');
});
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://i.postimg.cc/JhDZRG6C/IMG-20260425-WA0051.jpg'; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 CUTE-MARKHOR-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ CUTE-MARKHOR-MD Bot connected to WhatsApp!");
      ...
      const aliveMsg = `*╭──────────────●●►*\n> *CUTE-MARKHOR-MD CONNECTED SUCCESSFULLY*\n\n> *Type ${prefix}menu to view commands*  \n\n*╭⊱✫ KIRA MD ✫⊱╮*\n*│✫📂 Bot Name: ${botConfig.BOT_NAME}*\n*│✫🛡️ Owner: ${config.OWNER_NAME}*\n*│✫♻️ Prefix: ${prefix}*\n*│✫🌍 Mode: ${config.MODE}*\n*│✫⏰ Uptime: ${runtime(process.uptime())}*\n*╰──────────────●●►*\n\n> Enjoy Using KIRA MD`;
      
      // Image URL for connection message
      const imageUrl = 'https://i.postimg.cc/JhDZRG6C/IMG-20260425-WA0051.jpg';
      
      try {
        // Send to owner with image
        sock.sendMessage(ownerNumber[0] + '923151914309@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
      ...
      for (const participant of participants) {
        ...
        if (action === 'add') {
          ...
          if (settings.goodbye) {
            try {
              // Get user's profile picture
              const ppUrl = await getProfilePicture(sock, participant).catch(() => 'https://i.postimg.cc/JhDZRG6C/IMG-20260425-WA0051.jpg');
...
// BOT SALLERS KI MKC BY CUTE MARKHOR 